...
function MyMap() {
  ...
  function onFound({coords}) {
    if (currentLocation) { // Eliminate state change for minimal movement
      ...
      setCurrentLocation(coords)
      // initialLocation.push(coords.latitude)
      // initialLocation.push(coords.longitude)
      console.log("geolocation found", coords.latitude, coords.longitude)
  ...
  return (
    <>
      {currentLocation && testPins.length > 0
        ?   <>
                <MapContainer center={[40.7378961, -96.6426487]} zoom={18} minZoom={14}> {/* Map currently reloads on movement, potential problems with listening while moving */}
                  <MapComponent/>
                {testPins.map(pin => (
